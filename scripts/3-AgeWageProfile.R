...
# Create a quadratic age variable 
db <- db %>% mutate(age2= age^2)

# Run linear regressions for nominal and real income/wage as dependent variable

# ----- Model 2 & 3 for nominal hourly wage ------------------------------------

model2 <- lm(log_nominal_income  ~ age,  data= db) # linear model 

model3 <- lm(log_nominal_income  ~ age + age2, data= db) # quadratic model 

# ----- Model 4 & 5 for real hourly wage ------------------------------------

model4 <- lm(log_real_income  ~ age,  data= db) # linear model

model5 <- lm(log_real_income  ~ age + age2, data= db) # quadratic model

# Displaying model results for nominal hourly wage 
stargazer(model2, model3, type="text", covariate.labels=c("Age","Agesq"))

# Displaying model results for real hourly wage 
stargazer(model4, model5, type="text", covariate.labels=c("Age","Agesq"))

# Extracting residuals from each model
...
# Plotting residual density for each model
ggplot(data= db, 
...
# Plotting residuals vs observations to check for patterns in residuals
...
# Computing studentized residuals for model diagnostics and plotting results
...
# Removing outliers with absolute studentized residuals greater than 2
...
#Now that our db has studentized outliers, re-estimate models
...
stargazer(model2, model3, model4, model5, 
          ...
          dep.var.labels = c("Log Nominal Income", "Log Real Income"),
...
# Age at which income is maximized
...
## Finding the 'peak-age' with CI's according by bootstrapping the model for real wages
